package day01;

public class ArrayDemo7 {
	// 算標準差
	public static void main(String[] args) {
		int[] scoresA = { 100, 80, 60 };
		int[] scoresB = { 40, 45, 50 };

		// 求scoresA的平均
		int sumA = 0;
		for (int x : scoresA) {
			sumA = sumA + x;
		}
		int avgA = sumA / scoresA.length;

		// ----------------------------------

		// 求scoresB的平均
		int sumB = 0;
		for (int x : scoresB) {
			sumB = sumB + x;
		}
		int avgB = sumB / scoresB.length;

		// ----------------------------------
		System.out.println("scoresA 的平均:" + avgA);
		System.out.println("scoresB 的平均:" + avgB);
		// 計算 deltaA 與 sdA ----------------
		double deltaA = 0;
		for (int x : scoresA) {
			deltaA = deltaA + Math.pow(x - avgA, 2);
		}
		double sdA = Math.sqrt(deltaA / scoresA.length); // scoresA的標準差

		// 計算 deltaB 與 sdB ----------------
		double deltaB = 0;
		for (int x : scoresB) {
			deltaB = deltaB + Math.pow(x - avgB, 2);
		}
		double sdB = Math.sqrt(deltaB / scoresB.length); // scoresB的標準差
		// ----------------------------------
		System.out.println("scoresA 的標準差: " + sdA);
		System.out.println("scoresB 的標準差: " + sdB);
		// ----------------------------------

		// 判斷集中度
		if (sdA < sdB) {
			System.out.println("scoreA 集中度較高");
		} else {
			System.out.println("scoreB 集中度較高");
		}
	}
}
